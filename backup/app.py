...
from flaskr.settings import DBSERVER_SYSTEM, DBNAME_SYSTEM, IPFS_SERVER

from Server.flaskr.settings import OPERATIONS_DIR
...
def mint(nft:NFT,miner,owner,network,offchaindata_platform="IPFS",storagefile="",mail_new_wallet="",mail_existing_wallet=""):
  ...
  if "database" in network.lower() or "db-" in network.lower():
    ...
    if _dao.isConnected():
      rc=_dao.lazy_mint(nft,"",app.config["APPLICATION_ROOT"])
    else:
