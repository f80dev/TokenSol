  ...
  def nfts(self,service="fills",key="",value="",out="",timeout=0):
    ...
    if result.status_code==200:
      for nft in result.json()["result"]:
        ...
        if len(value)==0 or len(key)==0 or (compare_value in value and operator=="=") or (not compare_value in value and operator=="!="):
          if len(out)==0:
            rc.append(nft)
          else:
            new_nft=dict()
      ...
      return {"status":result.status_code,"message":result.content}
